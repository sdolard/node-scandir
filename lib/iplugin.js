...
exports.IPlugin = (function (){
	...
	*/
	...
	IPlugin.prototype.getPlatformRequirement = function(){
		this._eexception({
			code: 'EINTERFACE',
			message: 'IPlugin.prototype.setPlatformRequirement MUST BE overloaded'
		});
		return {
			'win32': {
				bin: 'dir',
	...
	IPlugin.prototype.getOutFile = function(file) {
		this._eexception({
			code: 'EINTERFACE',
			message: 'IPlugin.prototype.getOutFile MUST BE overloaded'
		});
	...
	IPlugin.prototype.getCommand = function(/*file*/) {
		this._eexception({
			code: 'EINTERFACE',
			message: 'IPlugin.prototype.getCommand MUST BE overloaded'
		});
		...
		exec(command, {
			...
			if (this._deleteInFileOnSuccess) {
				try{
					...
					this.log('deleted ("%s")', config.file);
				} catch(e){
					this.debug('failed deleting "%s"', config.file);
					this._eexception(e);
				}
			}
	...
	IPlugin.prototype.onScanEnd = function(/*totalCount, totalSize*/){
		...
		this._execOn({
			...
			stats: file.stats,
				...
				this.onDone(success, file.file, file.stats, outFile, function(){
					this.onScanEnd();
				}.bind(this));
			}.bind(this)
		});
